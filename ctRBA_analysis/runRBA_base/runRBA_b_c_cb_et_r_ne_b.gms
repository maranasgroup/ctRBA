...
$setGlobal settings "c_cb_r_ne_b"
$setGlobal identifier "b_c_cb_et_r_ne_b"
...
enforce_cell_syn(prosyn_cell)..		v(prosyn_cell) =e= prosyn_cell_rates(prosyn_cell) * mu * 0.99 * %nscale%;
...
*initialize a file for writing the results
FILE BISECTIONITER / 'results/%identifier%/iter_tracking_%identifier%.txt' /;
PUT BISECTIONITER;

PUT "iter",system.tab,"mu lb",system.tab,"mu",system.tab,"mu ub",system.tab,"delta mu",system.tab,"objective value",system.tab,"model status",system.tab,"solver status"/;

PUTCLOSE;

*while the RBA model status is infeasible, or the model bounds are not sufficiently close
loop(t$(((muUB - muLB > %tolerance%) or (%usemodel%.modelstat = 4)) and (iter < %out_iter_max%)),
	...
	iter_in = 0;
  	...
  	/*solve for that point*/
	while((((not %usemodel%.modelstat = 1) and (not %usemodel%.modelstat = 4)) or (iter_soln = 0)) and (iter_in < %inner_iter_max%),

		/*if we already have a solution, perturb mu*/
		if((iter_soln = 1),

			mu_temp = mu;
			mu = mu_temp + %tolerance%; 
	...
	delta_mu = muUB - muLB;
	PUT iter,system.tab,muLB:0:8,system.tab,mu:0:8,system.tab,muUB:0:8,system.tab,delta_mu:0:8,system.tab,(z.l*%nscaleback%):0:8,system.tab,%usemodel%.modelstat,system.tab,%usemodel%.solvestat/;

	PUTCLOSE;

  /*based on solution status update the bounds*/
	if ( %usemodel%.modelstat = 4 , 
		
		muUB = mu;

	elseif (%usemodel%.modelstat = 1),
		...
		/*if we get an unexpected model status just error out so we can address it*/
...
*now that we know the growth rate fix it and maximize product synthesis
mu = feas_mu;
...
PUT "maximum product objective value:",system.tab,(z.l*%nscaleback%):0:8/;
PUT "growth rate:",system.tab,mu:0:8/;
PUT "model status:",system.tab,%usemodel%.modelStat/;
PUT "solver status:",system.tab,%usemodel%.solveStat;
