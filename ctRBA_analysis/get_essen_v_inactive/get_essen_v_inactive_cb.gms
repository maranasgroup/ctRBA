...
$INLINECOM /*  */
$include "./get_essen_v_inactive_cb_settings.txt"
...
FILE statFile / 'results/cb/get_v_min_modelStat.txt' /;
PUT statFile;
...
file fluxFileMin / 'results/cb/get_essen_v_inactive_flux_min.txt' /;
PUT fluxFileMin;
...
file fluxFileAll / 'results/cb/get_essen_v_inactive_flux_all.txt' /;
PUT fluxFileAll;
...
*write a file for flux results all reactions including zeros
file fluxInactive / 'results/cb/get_essen_v_inactive_flux_inactive.txt' /;
PUT fluxInactive;
...
*write a file for flux results just those with flux
file FileInacMin / 'results/cb/minimal_essential_inactives.txt' /;
PUT FileInacMin;
...
u = sum(prosyn, S('BIO-protcyt', prosyn) * v.l(prosyn));

display u;

w = %mu% * sum(prosyn, NAA(prosyn) * v.l(prosyn));
